...
RSpec.describe Spl::Spree::Storefront::CheckoutControllerDecorator, type: :controller do
  controller(ActionController::Base) do
    ...
    def activate_coupon = super
    def deactivate_coupon = super
  ...
  describe "#load_user_coupons" do
    ...
    it "does not run load_user_coupons as a before_action for activate_coupon (it is called inside the action instead)" do
      allow(PromotionSwitcherService)
        .to receive(:new)
              .and_return(instance_double("PromotionSwitcherService", call: true))
      ...
      post :activate_coupon, params: { coupon_code: "ABC123" }, format: :html
    end
  end
  ...
  describe "#activate_coupon" do
    ...
    context 'when coupon is invalid' do
      ...
      it "still redirects even if ActivateCouponService raises" do
        ...
        expect(activate).to receive(:call).and_raise(Spl::Coupons::ActivateCouponService::ActivateCouponServiceError, "some error")
  ...
  describe "#deactivate_coupon" do
    ...
    it "still redirects even if DeactivateCouponService raises" do
      ...
      expect(deactivate).to receive(:call).and_raise(Spl::Coupons::DeactivateCouponService::DeactivateCouponServiceError, "some error")
